...
object AudioUtils {
    ...
    // Hard limit audio to 10 seconds to spare memory
    private const val MAX_AUDIO_DURATION_SEC = 10

    @SuppressLint("MissingPermission")
    suspend fun recordAudio(onMaxDurationReached: () -> Unit, stopSignal: suspend () -> Boolean): ByteArray = withContext(Dispatchers.IO) {
        ...
        try {
            while (recorder.recordingState == AudioRecord.RECORDSTATE_RECORDING && !stopSignal()) {
                ...
                if (System.currentTimeMillis() - startMs >= MAX_AUDIO_DURATION_SEC * 1000) {
